...
void transmitSysEx(unsigned int len, const uint8_t *sysexarray, byte inPort) {
    ...
    if (sysexarray[0] == 240 && sysexarray[1] == 126 && sysexarray[3] == 6 && sysexarray[4] == 2) { // SysExID reply
        ...
        midiRouter::MRInputPort *inport = router.inputAt(inPort);
        midiRouter::MROutputPort *outport = router.outputAt(inPort);
...
void profileInstruments() {
    ...
    for (int i = 0; i < 10; i++) {
        ...
        if (prod != "") {
            ...
            midiRouter::MRInputPort *inport = router.inputAt(portIndex);
            midiRouter::MROutputPort *outport = router.outputAt(portIndex);
            strncpy(inport->name, shortName, 6);
            strncpy(outport->name, shortName, 6);
        }
    }
